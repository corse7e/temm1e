...
## F-01: `skyclaw-core` config module makes core heavier than intended

**Severity:** Medium
**Category:** Dependency direction
**Location:** `crates/skyclaw-core/Cargo.toml`, `crates/skyclaw-core/src/config/`
...
**Recommendation:** Extract `config/loader.rs` and `config/env.rs` into a separate crate (e.g., the root binary or a new `skyclaw-config` crate), keeping only the `SkyclawConfig` struct and related types in core. This would reduce core's dependency count from 13 to approximately 8.

---

## F-02: `skyclaw-gateway` depends on `skyclaw-agent`, violating strict "impl depends on core only"

**Severity:** Medium
**Category:** Dependency direction
**Location:** `crates/skyclaw-gateway/Cargo.toml`

`skyclaw-gateway` imports `skyclaw-agent` for `AgentRuntime`. This creates a cross-impl-crate dependency:
```
gateway -> agent -> core
```
...
**Recommendation:** Define an `AgentRuntime`-like trait in `skyclaw-core` that the gateway depends on. The actual `AgentRuntime` struct would implement this trait in `skyclaw-agent`, and the binary crate would wire them together via dependency injection.
...
**Location:**
- `crates/skyclaw-core/src/config/loader.rs:29,34` -- `std::fs::read_to_string`
- `crates/skyclaw-core/src/config/loader.rs:33` -- `path.exists()` (blocking stat)
- `crates/skyclaw-channels/src/cli.rs:89` -- `std::fs::metadata`
- `crates/skyclaw-vault/src/local.rs:96` -- `std::fs::Permissions::from_mode` (not blocking itself, but the pattern around it)
...
**Location:**
- `crates/skyclaw-core/src/config/env.rs:5` -- `Regex::new(...).expect("invalid regex")`
- `crates/skyclaw-vault/src/detector.rs:38-103` -- Multiple `Regex::new(...).unwrap()` inside `LazyLock`
- `crates/skyclaw-vault/src/detector.rs:121,135,136` -- `caps.get(N).unwrap()` in regex capture processing
...
**Location:** `crates/skyclaw-providers/src/lib.rs:7` -- `#![allow(dead_code)]`

The crate-level `#![allow(dead_code)]` suppresses all dead-code warnings for the entire `skyclaw-providers` crate. This hides legitimate unused code and reduces Rust's ability to detect stale code over time.
...
**Location:** `crates/skyclaw-gateway/src/session.rs:87-91`
...
## F-07: `SkyclawError` does not implement `From` for `sqlx::Error`

**Severity:** Medium
**Category:** Error handling
**Location:** `crates/skyclaw-core/src/types/error.rs`

`SkyclawError` implements `From<serde_json::Error>` and `From<std::io::Error>` via `#[from]`, but does not have a `From<sqlx::Error>` variant. This means every sqlx call in `skyclaw-memory` requires a manual `.map_err()`. This is not inherently wrong (it keeps core independent of sqlx), but it leads to verbose and repetitive error mapping throughout `sqlite.rs`.

**Recommendation:** This is acceptable for keeping `skyclaw-core` free of sqlx dependency. Consider a helper function in `skyclaw-memory` (e.g., `fn sql_err(e: sqlx::Error) -> SkyclawError`) to reduce boilerplate.
...
**Location:** `crates/skyclaw-vault/src/detector.rs`, `crates/skyclaw-agent/src/runtime.rs`

ADR-004 specifies credential detection must run "on every incoming message." The `detect_credentials` function is implemented and well-tested in `skyclaw-vault`, but it is never called from:
...
**Location:**
- `crates/skyclaw-memory/src/sqlite.rs:228-246`
- `crates/skyclaw-memory/src/markdown.rs:300-319`
...
**Recommendation:** Move these helpers to a shared location within `skyclaw-memory` (e.g., the `lib.rs` or a new `util.rs` module), or consider adding `Display` / `FromStr` implementations on `MemoryEntryType` in `skyclaw-core`.
...
**Location:** `crates/skyclaw-providers/src/openai_compat.rs:237-253`
...
**Severity:** Low
**Category:** Trait implementation correctness
**Location:** `crates/skyclaw-channels/src/telegram.rs:318`
...
**Severity:** Low
**Category:** Error handling
**Location:** `crates/skyclaw-core/src/types/error.rs:44-45`

This means any `serde_json::Error` anywhere in the codebase can be auto-converted to `SkyclawError::Serialization` via `?`. This is convenient but slightly imprecise -- a JSON error during provider response parsing would show up as "Serialization error" rather than "Provider error". The current code correctly uses `.map_err()` in most places where a more specific error is appropriate, so this is just a minor ergonomics note.
...
**Location:** `crates/skyclaw-core/src/types/mod.rs`, `crates/skyclaw-core/src/lib.rs`

`lib.rs` does `pub use types::*;` which re-exports the sub-modules (`message`, `file`, `config`, `session`, `error`) but not their individual types. This means consumers must write `skyclaw_core::types::error::SkyclawError` or `skyclaw_core::error::SkyclawError` rather than `skyclaw_core::SkyclawError`.

The `traits/mod.rs` does `pub use provider::*;` etc., so trait names are available at `skyclaw_core::Provider`. But type names require the module path.

This is inconsistent: traits are importable as `skyclaw_core::Provider` but types require `skyclaw_core::types::error::SkyclawError`. Some crates use `skyclaw_core::error::SkyclawError` (memory), others use `skyclaw_core::types::error::SkyclawError` (agent, gateway, providers, channels, vault).

**Recommendation:** Add `pub use error::SkyclawError;` and similar re-exports in `types/mod.rs` to allow `skyclaw_core::SkyclawError` directly. Alternatively, standardize the import path across all consuming crates.
...
The `Commands::Start`, `Commands::Chat`, `Commands::Migrate`, and all `SkillCommands` contain TODO comments and placeholder `println!` statements. The wiring between the binary and the impl crates is not yet connected -- `skyclaw-gateway::SkyGate` is not instantiated, providers/channels are not created from config, etc.
...
**Location:** `crates/skyclaw-core/src/types/config.rs:82-87`
...
The vault URI scheme (`vault://skyclaw/key`) exists to address this, but nothing in the config loading path resolves vault URIs. A user who sets `api_key = "vault://skyclaw/anthropic_api_key"` would send the literal string "vault://..." as their API key.
...
**Location:** `crates/skyclaw-channels/src/telegram.rs:67-79`, `crates/skyclaw-channels/src/telegram.rs:288-297`
...
4. **Error handling** is generally strong -- `SkyclawError` is used consistently, `unwrap()` in lib code is limited to known-safe patterns (regex literals) and regex captures.
