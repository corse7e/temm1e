# SkyClaw Architecture Conformance Checklist

**Reviewer:** Code Reviewer (T6b)
**Date:** 2026-03-08
**Codebase snapshot:** All implemented source files in `/Users/quanduong/Documents/Github/skyclaw/crates/`
...
|---|-------------|---------|----------|
| 1.1 | 13-crate workspace | **PASS** | Root `Cargo.toml` `[workspace]` members lists 12 crates + the root binary = 13 total. All 13 names match the ADR exactly: `skyclaw-core`, `skyclaw-gateway`, `skyclaw-agent`, `skyclaw-providers`, `skyclaw-channels`, `skyclaw-memory`, `skyclaw-vault`, `skyclaw-tools`, `skyclaw-skills`, `skyclaw-automation`, `skyclaw-observable`, `skyclaw-filestore`, plus root `skyclaw` binary. |
| 1.2 | Feature flags gate optional components | **PASS** | Root `Cargo.toml` defines 6 feature flags (`telegram`, `discord`, `slack`, `whatsapp`, `browser`, `postgres`) that propagate to downstream crates. `skyclaw-channels/Cargo.toml` gates `teloxide`, `serenity`, `poise`, `reqwest` behind features. `skyclaw-tools` gates `chromiumoxide`. `skyclaw-filestore` gates `aws-sdk-s3`. `skyclaw-memory` gates `postgres`. |
| 1.3 | No circular dependencies | **PASS** | Dependency graph is strictly hierarchical. `skyclaw-core` depends on no internal crates. `skyclaw-agent` depends only on `skyclaw-core`. `skyclaw-gateway` depends on `skyclaw-core` + `skyclaw-agent`. All other impl crates (`providers`, `channels`, `memory`, `vault`, `tools`, `skills`, `automation`, `observable`, `filestore`) depend only on `skyclaw-core`. Binary crate depends on all of them. No cycles. |
| 1.4 | `skyclaw-core` is dependency-free (beyond serde/async-trait) | **PARTIAL** | ADR says "zero external deps beyond serde/async-trait". The actual `skyclaw-core/Cargo.toml` has 13 external dependencies: `async-trait`, `serde`, `serde_json`, `toml`, `serde_yaml`, `thiserror`, `bytes`, `uuid`, `chrono`, `url`, `futures`, `tokio-stream`, `regex`, `dirs`. Many are justified by the types they define (e.g., `bytes::Bytes` in file types, `chrono` in timestamps), but `regex`, `dirs`, `toml`, `serde_yaml` exist only because the config loader module lives in core. This is heavier than the ADR's stated goal. |
...
| # | Requirement | Verdict | Evidence |
|---|-------------|---------|----------|
| 2.1 | 12 core traits defined in `skyclaw-core` | **PASS** | Exactly 12 traits defined in `crates/skyclaw-core/src/traits/`: `Provider`, `Channel`, `Tool`, `Memory`, `Tunnel`, `Identity`, `Peripheral`, `Observable`, `FileStore`, `Vault`, `Orchestrator`, `Tenant`. Each in its own module file, aggregated via `traits/mod.rs`. |
...
| 2.3 | Impl crates depend on core only | **PARTIAL** | `skyclaw-providers`, `skyclaw-channels`, `skyclaw-memory`, `skyclaw-vault` all depend on `skyclaw-core` and no other internal crates. However, `skyclaw-gateway` depends on both `skyclaw-core` and `skyclaw-agent`, which is a cross-impl-crate dependency. This is architecturally reasonable (gateway routes through the agent runtime) but technically violates the strict "impl crates depend on core only" rule. |
...
| 4.3 | Credential detection on incoming messages | **PASS** | `skyclaw_vault::detector::detect_credentials` implements regex-based scanning for Anthropic (`sk-ant-*`), OpenAI (`sk-*`), Groq (`gsk_*`), Google (`AIza*`), Slack (`xoxb-*`, `xoxp-*`), plus generic patterns (`api_key=`, `token=`, `secret=`, env-var style). Includes a comprehensive test suite. |
| 4.4 | File transfer utilities (save, read) | **PASS** | `skyclaw-channels/src/file_transfer.rs` provides `save_received_file` (with path-traversal sanitization) and `read_file_for_sending` with MIME type detection. |
| 4.5 | Credential detection wired into message flow | **PARTIAL** | The `detect_credentials` function exists in `skyclaw-vault`, but it is not called from the message processing pipeline in `skyclaw-agent` or `skyclaw-gateway`. The ADR specifies detection should run "on every incoming message." This integration is not yet implemented. |
...
| 5.4 | `SkyclawError` includes security-specific variants | **PASS** | `SkyclawError` has `PermissionDenied`, `SandboxViolation`, `Auth`, `RateLimited` variants -- all security-relevant error types. |
...
| 6.9 | thiserror for library errors | **PASS** | `SkyclawError` in core uses `#[derive(Error)]` from `thiserror 2`. |
...
| 6.13 | opentelemetry for observability | **N/A** | `opentelemetry 0.27` + `tracing-opentelemetry 0.28` declared in workspace deps. `skyclaw-observable` is a stub. |
