...
| `docker-compose.yml` | Local dev stack: skyclaw service + optional PostgreSQL |
...
- **Stage 4 (runtime):** `alpine:3.19` with only curl + ca-certificates. Non-root `skyclaw` user. Copies binary + `config/default.toml`. Exposes 8080, curl-based health check.

Build profile from Cargo.toml already applies: `opt-level = "z"`, LTO, `codegen-units = 1`, `strip = true`, `panic = "abort"`.

## docker-compose.yml

- **skyclaw** service: port 8080, env vars (SKYCLAW_MODE, ANTHROPIC_API_KEY, TELEGRAM_BOT_TOKEN), volume for `~/.skyclaw` data, health check.
