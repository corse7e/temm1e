...
//!
//! Tokens are stored in `~/.skyclaw/oauth.json`. Access tokens expire in ~1 hour
...
impl TokenStore {
    ...
    /// Load tokens from ~/.skyclaw/oauth.json
    pub fn load() -> Result<Self, SkyclawError> {
        let path = Self::default_path();
        let content = std::fs::read_to_string(&path).map_err(|e| {
            SkyclawError::Auth(format!(
                "No OAuth tokens found at {}. Run `skyclaw auth login` first. ({})",
                path.display(),
                e
            ))
        })?;
        let tokens: CodexOAuthTokens = serde_json::from_str(&content)
            .map_err(|e| SkyclawError::Auth(format!("Failed to parse OAuth tokens: {}", e)))?;
        Ok(Self {
            path,
            tokens: Mutex::new(tokens),
            client: reqwest::Client::new(),
    ...
    pub fn save_to_disk(&self, tokens: &CodexOAuthTokens) -> Result<(), SkyclawError> {
        ...
        let content = serde_json::to_string_pretty(tokens)
            .map_err(|e| SkyclawError::Auth(format!("Failed to serialize tokens: {}", e)))?;
        std::fs::write(&self.path, content)
            .map_err(|e| SkyclawError::Auth(format!("Failed to write tokens: {}", e)))?;
    ...
    /// Default path: ~/.skyclaw/oauth.json
    ...
    pub fn delete() -> Result<(), SkyclawError> {
        ...
        if path.exists() {
            std::fs::remove_file(&path)
                .map_err(|e| SkyclawError::Auth(format!("Failed to delete tokens: {}", e)))?;
        }
        Ok(())
    }
